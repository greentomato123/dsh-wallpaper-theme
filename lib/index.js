...
/** Accepted upload extensions and their media content types (images + videos). */
...
/** 100 MiB upload cap — MP4/WebM wallpapers can be large. */
...
function imageContentType(imagePath) {
  return MEDIA_TYPES[extname(imagePath).toLowerCase()] ?? "image/png";
}
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === "POST" && pathname === `${prefix}/api/upload`) {
          ...
          if (!MEDIA_TYPES[ext]) {
            sendJson(400, { ok: false, error: `unsupported media type "${ext || "(none)"}" — use png/jpg/jpeg/webp/gif/mp4/webm` });
            return;
          }
